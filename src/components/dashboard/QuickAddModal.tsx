...
interface QuickAddModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { amount: number; date: Date; wallet: string; description: string; category: string; name: string }) => void;
  category: string;
...
const QuickAddModal = ({ 
  ...
  // Get the currency symbol from selected wallet or fallback to prop/active wallet
  const selectedWallet = wallets.find(w => w.id === wallet);
  const currencySymbol = selectedWallet?.currencySymbol || propCurrencySymbol;

  // Auto-select active wallet when modal opens
  useEffect(() => {
    if (isOpen && !wallet) {
      const activeWallet = wallets.find(w => w.isActive);
      if (activeWallet) {
        setWallet(activeWallet.id);
      }
    }
  }, [isOpen, wallets, wallet]);
    ...
    <Dialog open={isOpen} onOpenChange={handleCancel}>
      <DialogContent className="p-0 gap-0 overflow-hidden max-h-[90vh] overflow-y-auto [&>button]:hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-popover sticky top-0 z-10">
          ...
          <h3 className="font-semibold text-base capitalize">{category.replace("-", " ")}</h3>
          <Button 
            variant="ghost" 
            size="sm" 
            onClick={handleSave}
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <input
              ...
              className="w-24 h-11 rounded-xl border-transparent bg-secondary/50 px-2 text-base ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring shrink-0"
            />
          </div>
          ...
          <Select value={wallet} onValueChange={setWallet}>
            <SelectTrigger className="w-full rounded-xl h-11 border-transparent bg-secondary/50 text-base">
          ...
          <Input
            ...
            placeholder="Name (shown in transactions)"
            className="rounded-xl h-11 border-transparent bg-secondary/50 text-base"
          />
          ...
          <Textarea
            ...
            className="rounded-xl resize-none border-transparent bg-secondary/50 text-base"
            rows={3}
          />
        </div>
      </DialogContent>
